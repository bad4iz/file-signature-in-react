...
exports.IFCPlugin = IFCPlugin;
exports.IFCError = void 0;
...
function IFCCrypto(resultData) {
  ...
  this.getCryptoId = function () {
    if (resultData["type"] === IFCConst.IFC_CRYPTO_PKCS11) resultData["crypto_id"] = resultData["alias"] + "/" + resultData["num"];else if (resultData["type"] === IFCConst.IFC_CRYPTO_CAPI || resultData["type"] === IFCConst.IFC_CRYPTO_CAPI_LINUX) resultData["crypto_id"] = resultData["alias"];
  ...
  this.getExtendedKeyUsage = function () {
    ...
    var eku = [{
      alias: "JaCarta",
      oid: "1.2.643.3.205.110.1"
    }, {
      alias: "ruTokenECP",
      oid: "1.2.643.3.205.110.7"
    }, {
      alias: "VIPNet",
      oid: "1.2.643.3.205.110.6"
    }, {
      alias: "SignalCom",
      oid: "1.2.643.3.205.110.8"
    }, {
      alias: "LISSI-CSP",
      oid: "1.2.643.3.205.110.9"
    }, {
      alias: "CryptoPro",
      oid: "1.2.643.3.205.110.2"
    }];
  ...
  this.getCertificatePolicies = function () {
    ...
    var cp = [{
      alias: "JaCarta",
      oid: IFCConst.OID_KC1 + "," + IFCConst.OID_KC2
    }, {
      alias: "ruTokenECP",
      oid: IFCConst.OID_KC1 + "," + IFCConst.OID_KC2
    }, {
      alias: "CryptoPro",
      oid: IFCConst.OID_KC1
    }, {
      alias: "VIPNet",
      oid: IFCConst.OID_KC1
    }, {
...
function IFCCertificateInfo(info) {
  ...
  this.getPrintable = function (cr, tab, sep) {
    ...
    var certPrintable = "Номер квалифицированного сертификата: " + this.getSerialNumber() + cr + "Действие квалифицированного сертификата:" + cr + tab + tab + "с " + this.getValidFrom() + cr + tab + tab + "по " + this.getValidTo() + cr + cr + tab + "Сведения о владельце квалифицированного сертификата" + cr + cr + "Фамилия, имя, отчество: " + this.getSubjectDN().getCommonName() + cr + "Страховой номер индивидуального лицевого счета: " + this.getSubjectDN().getSNILS() + cr + cr + tab + "Сведения об издателе квалифицированного сертификата" + cr + cr + "Наименование  удостоверяющего  центра: " + this.getIssuerDN().getCommonName() + cr + "Место  нахождения  удостоверяющего центра: " + this.getIssuerDN().getCountryName() + sep + this.getIssuerDN().getStateOrProvinceName() + sep + this.getIssuerDN().getLocalityName() + sep + this.getIssuerDN().getStreetAddress() + cr; // Optional information in issuerDN
    ...
    certPrintable += tab + "Расширения сертификата" + cr + cr + this.getExtensionsString().replace(/(\r\n|\n|\r)/gm, cr).replace(/^\s+/mg, tab);
...
function IFCDN(dnString, dnSeparator) {
  /**@private*/
  var getNumericOid = function getNumericOid(oid) {
  ...
  this.getSubjectArray = function () {
    ...
    for (var dnKey in this.dnData) {
      oidsCount++; //dnArray.push({"oid": dnKey, "oid_type": IFCConst.getDNDataType(dnKey), "value": this.dnData[dnKey]});
  ...
  this.getOneLine = function () {
    var Line = "";
    if (this.getCountryName()) Line += "c=" + this.getCountryName() + ", ";
    if (this.getStateOrProvinceName()) Line += "stateorprovincename=" + this.getStateOrProvinceName() + ", ";
    if (this.getLocalityName()) Line += "l=" + this.getLocalityName() + ", ";
    if (this.getCommonName()) Line += "cn=" + this.getCommonName() + ", ";
    if (this.getSurname()) Line += "surname=" + this.getSurname() + ", ";
    if (this.getGivenName()) Line += "givenname=" + this.getGivenName() + ", ";
    if (this.getStreetAddress()) Line += "street=" + this.getStreetAddress() + ", ";
    if (this.getOrganizationName()) Line += "o=" + this.getOrganizationName() + ", ";
    if (this.getOrganizationUnitName()) Line += "ou=" + this.getOrganizationUnitName() + ", ";
    if (this.getTitle()) Line += "title=" + this.getTitle() + ", ";
    if (this.getEmailAddress()) Line += "email=" + this.getEmailAddress() + ", ";
    if (this.getOGRN()) Line += "ogrn=" + this.getOGRN() + ", ";
    if (this.getSNILS()) Line += "snils=" + this.getSNILS() + ", ";
    if (this.getINN()) Line += "inn=" + this.getINN() + ", ";
...
function IFCHash(hashBase64) {
  ...
  var binStringToHex = function binStringToHex(s) {
    var s2 = [],
        c;
    var result = "";
...
var IFCConst = {
  ...
  /**@private*/
  getDNDataType: function getDNDataType(oidName) {
    var dataTypeValue;
  ...
  /**@private*/
  emptyString: function emptyString(str) {
...
var IFCError = {
  ...
  IFC_P11_LOGIN_ERROR: 0x00A0,
  // Equals to CKR_PIN_INCORRECT
  ...
  getErrorDescription: function getErrorDescription(error_code) {
    switch (error_code) {
      // Plugin errors
...
function IFCPlugin(pluginObject, inGoogleChrome, testFunctionsEnabled) {
  if (_typeof(pluginObject) === _typeof(true)) {
  ...
  var libVersion = "2.2.1"; // Префикс для GUID

  /**@private*/

  var guidPrefix = ""; // Коннектор для общения с плагином

  /**@private*/

  var connector = null; // Статус библиотеки

  /**@public*/

  this.status = null; // Схема с помошью которой можно окрыть расширение для safari
  ...
  function IFCNPAPIConnector() {
    ...
    this.send = function (msg, callback) {
      try {
        ...
        callback({
          "error_code": IFCError.IFC_PLUGIN_UNDEFINED_ERROR
        });
      }
    };
  } ///////////////////////////////////////////////
  ...
  function IFCNativeMessagingConnector() {
    ...
    /**@private*/
    var WND = window; // Текущий пользовательский callback
    ...
    function IFCMessageHandler(event) {
      ...
      } catch (e) {//Пропускаем невалидные сообщения
      }
    }
    /**@private
    ...
    function connectToIFC() {
      if (connected) return IFCError.IFC_OK;
    ...
    this.send = function (msg, callback) {
      ...
      if (currentUserCallback) {
        callback({
          "error_code": IFCError.IFC_GENERAL_ERROR
        });
        return;
  ...
  if (isNativeMessaging()) connector = new IFCNativeMessagingConnector();else connector = new IFCNPAPIConnector(); ///////////////////////////////////////////////
  ...
  function getCryptoById(cryptoId, callback) {
    getCryptoList(function (msg) {
      ...
      callback({
        "error_code": IFCError.IFC_CRYPTO_NOT_FOUND,
        "crypto": crypto
      });
    });
  }
      ...
      getCertificateHandleFromString(peerCertificate, function (msg) {
        ...
        connector.send({
          ...
          freeCertificateHandle(x509Handle, function (msg) {
            if (IFCError.IFC_OK === msg.error_code) callback({
              "error_code": err_code,
  ...
  function getHashByDataType(containerId, dataType, data, callback) {
    connector.send({
      ...
      callback({
        "error_code": IFCError.IFC_OK,
        "hash": new IFCHash(msg.hashValue)
      });
    });
  }
    ...
    getCryptoList(function (msg) {
      ...
      connector.send({
        ...
        for (var i = 0; i < msg.result_array.length; i++) {
          var cur_crypto = false; // Находим объект IFCCrypto по ID
          ...
          if (cur_crypto === false) continue; // Находим сертификаты для этого СКЗИ
  ...
  function getCertificateListByCryptoId(cryptoId, callback) {
    getCryptoById(cryptoId, function (msg) {
      ...
      connector.send({
        func_name: "get_list_certs",
        params: {
          "cryptoID": crypto.getCryptoId()
        }
      }, function (msg) {
        if (IFCError.IFC_OK !== msg.error_code) {
          callback(msg);
  ...
  function getKeyListByCryptoId(cryptoId, userPin, callback) {
    getCryptoById(cryptoId, function (msg) {
      ...
      if (!(userPin && userPin.length > 0) && crypto.isPKCS11()) {
        callback({
          "error_code": IFCError.IFC_P11_INVALID_PIN_ERROR
        });
      }
  ...
  function getCertificateHandleFromString(dataString, callback) {
    connector.send({
      ...
      params: {
        "cert": dataString,
        "cert_data_type": IFCConst.IFC_CERT_UNKNOWN
      }
    }, callback);
  }
  ...
  function getCertificateHandleFromContainerID(containerId, callback) {
    connector.send({
      func_name: "load_x509_from_container",
      params: {
        "containerId": containerId
      }
    }, callback);
  ...
  function getCertificateFromString(dataString, callback) {
    getCertificateHandleFromString(dataString, function (msg) {
      ...
      var x509Handle = msg.x509Handle; // Set the certificate to a container
      ...
      getCertificateByHandle(x509Handle, function (msg) {
        ...
        var cert = msg.cert; // Free certificate handle
  ...
  function getCertificate(containerId, callback) {
    getCertificateHandleFromContainerID(containerId, function (msg) {
      ...
      connector.send({
        ...
        var cert = new IFCCertificateInfo(msg.cert_info); // Free certificate
  ...
  function encrypt(containerId, userPin, peerCertificateBase64, data, callback) {
    ...
    getCertificateHandleFromString(peerCertificateBase64, function (msg) {
      ...
      var peer_x509Handle = msg.x509Handle;
      connector.send({
        func_name: "encrypt",
        params: {
          "containerId": containerId,
        ...
        freeCertificateHandle(peer_x509Handle, function (msg) {
          ...
          if (IFCError.IFC_OK === err_code) callback({
            "error_code": err_code,
            "encrypted": result
          });else callback({
            "error_code": err_code
  ...
  function pkcs11MoveContainer(oldContainerId, newContainerId, userPin, callback) {
    ...
    getCryptoById(getCryptoIdByContainerId(oldContainerId), function (msg) {
      ...
      if (msg.crypto == null || !msg.crypto.isPKCS11()) {
        callback({
          "error_code": IFCError.IFC_BAD_PARAMS
        });
        return;
